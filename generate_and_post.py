...
# Нішування: тримати в усіх промптах як приховану лінзу, не як фразу для буквального вставлення
NICHE = (
...
UMBRELLAS = [
    ...
    [
        "коли ти завжди опора для інших — а хто опора для тебе",
        "як розуміння проблеми не завжди веде до зміни поведінки",
        "чому «я все встигаю» не означає «мені добре»",
        "право не заслуговувати любов щодня",
        "коли турбота про інших стає втечею від себе",
        "як помітити, що ти загубив(ла) себе в ролі",
        "повернення собі права просто бути, а не лише бути корисним",
    ],
]
...
STAGE_INSTRUCTIONS = {
    ...
    "довіра": ("Тип: ДОВІРА. Розкрий тему «{theme}» глибше: поясни психологічний механізм простими "
               ...
               "Можеш показати, чому просте розуміння проблеми ще не змінює поведінку — "
               "керують звичні емоційні способи виживати, а не логіка."),
...
# Банк візуальних прийомів для фейслес-відео (фонова частина без обладнання)
FACELESS_STYLES = [
...
# Банк виробничих сценаріїв із наявним обладнанням (Nikon, vlog-камера, мікрофон, проектор, світло)
PROD_STYLES = [
...
HOOK = ("\nГАЧОК: перший рядок чіпляє за секунду — несподіване спостереження, питання або сцена, "
        "у якій читач впізнає СЕБЕ як людину, що звикла бути опорою для інших. НЕ починай з визначення.\n")
...
def generate_video(theme):
    ...
    prompt = (
        ...
        "📘 Підпис під Facebook (тон тепліший, розмовний, без хештегів або 1 максимум):"
...
def generate_carousel(theme):
    prompt = (
        ...
        "📘 ПІДПИС ПІД FACEBOOK: той самий зміст, тепліший тон, без хештегів або 1 максимум."
        + ANTISHABLON + "\nЖива людська українська."
    )
    return ask_gemini(prompt)
