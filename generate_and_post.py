...
def generate_video(theme):
    prompt = (
        ...
        "🙈 ВАРІАНТ Б — ФЕЙСЛЕС\nОзвучка за кадром або текст на екран. Кадри + повний текст.\n\n"
...
def generate_carousel(theme):
    prompt = (
        ...
        "(6 слайдів). Кожен слайд — мінімум тексту, легко читається.\n"
        + ACCURACY + HOOK +
        ...
        "СЛАЙД 3: ще аспект + 1-2 рядки.\n"
        "СЛАЙД 4: ще аспект + 1-2 рядки.\n"
        "СЛАЙД 5: головна думка/висновок.\n"
        "СЛАЙД 6 (CTA): один заклик — зберегти, написати в коментарі, і запрошення: "
        "«Більше — у телеграм-каналі: " + CHANNEL_LINK + "».\n\n"
        "📲 ПІДПИС ПІД ПОСТ: перші 2 рядки — гачок (Instagram показує лише їх!), далі 2-3 речення розкриття, "
...
def send_support_content(theme, stage):
    ...
    # Раз на 4 дні — карусель для Instagram
    if day % 4 == 1:
    ...
    # На залученні інколи тест/образ/опитування
    if stage == "залучення":
        roll = random.random()
        if roll < 0.2:
            try:
