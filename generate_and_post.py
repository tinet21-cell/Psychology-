...
# Один конкретний "біль" на тиждень. Дні всередині — РІЗНІ КУТИ того самого болю,
# а не нові теми (за принципом: глибина краще за широту).
UMBRELLAS = [
    [
...
ANTISHABLON = (
    ...
    "«Спойлер:», «Памʼятай: ти не один», «Кожен з нас»; драматичні обриви через крапку; "
...
ENGAGE = (
    "\nЗАЛУЧЕННЯ: наприкінці — конкретна пропозиція, а не загальний заклик. "
    "Приклади структури (не копіюй слово в слово, формулюй живо): «якщо впізнав(ла) себе — "
    "напиши слово [конкретне слово] у комментарі/дірект, і я надішлю [конкретну вправу/розбір]» "
    "АБО пряме практичне питання, що чекає відповіді в коментарях. "
    "НЕ обмежуйся фразою «поділись у коментарях» без конкретики.\n"
)
